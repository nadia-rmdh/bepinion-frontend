    ...
    return (
        <Modal isOpen={isOpen} toggle={() => handleToggle()} size="lg">
            <ModalBody className="py-4 px-3" style={{ minHeight: '90vh' }}>
                ...
                {data?.content.template === 'c8' && <CrazyEightCardDetail socket={socket} cardId={data.content.id} container={data.content.container} members={members} write={status === 'ideation'} />}
                {data?.content.template === 'fishbone' && <FishBoneDetail socket={socket} cardId={data.content.id} container={data.content.container} members={members} write={status === 'ideation'} />}
