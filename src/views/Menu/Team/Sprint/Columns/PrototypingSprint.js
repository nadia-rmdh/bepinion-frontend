...
export default memo(({ title, socket, column, cards, members, status, leadId }) => {
    ...
    const sprint = useMemo(() => {
        return [getItems(cards.filter(card => card.category === 'todo')[0]?.cards ?? []), getItems(cards.filter(card => card.category === 'inprogress')[0]?.cards ?? []), getItems(cards.filter(card => card.category === 'done')[0]?.cards ?? [])]
    }, [cards, getItems])
