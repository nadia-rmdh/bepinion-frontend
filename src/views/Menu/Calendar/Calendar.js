    ...
    modalDetailEvent = (event) => {
        ...
        if (event.personnel !== null) {
            event.personnel.map(data => {
                ...
                // this.state.personnels.map(data => {
                //     if (data1.id === data.value) {
                //         label = data.fullName;
                //     }
                // })
    ...
    addDataToAPI = () => {
        ...
        for (let field of requiredFields) {
            if (!this.state.event[field]) {
                toast.error('Semua isian harus diisi.');
                return;
            }
        }
    ...
    render() {
        ...
        return (
            <div className="animated fadeIn">
                <h4 className="content-title mb-4">{t('kalender')} {t('perusahaan')}</h4>
                ...
                <Modal isOpen={this.state.modalDetailEvent} toggle={this.cancelModalDetailEvent} className={this.props.className} >
                    <ModalBody>
                        <div className="d-flex justify-content-between align-items-center mb-4">
                            <h5 className="content-sub-title mb-0">Details Agenda</h5>
