...
class PengaturanDasarPernikahan extends Component {
    ...
    componentDidMount() {
        axios.get(process.env.REACT_APP_DOMAIN + `/api/v1/master/marital-status?all=true`, { headers: { "Authorization": `Bearer ${this.state.session}` } })
            .then(res => {
                const martialStatus = res.data.data;
                this.setState({ martialStatus });
            })
    ...
    handleChange = (event) => {
        ...
        if (event.target.checked === true) {
            axios.post(process.env.REACT_APP_DOMAIN + '/api/v1/master/marital-status/enable/' + id, null, { headers: { "Authorization": `Bearer ${this.state.session}` } })
                .then(res => {
                    console.log(res);
            ...
            axios.post(process.env.REACT_APP_DOMAIN + '/api/v1/master/marital-status/disable/' + id, null, { headers: { "Authorization": `Bearer ${this.state.session}` } })
                .then(res => {
                    console.log(res);
                    this.setState(state => ({ checkedItems: state.checkedItems.set(item, isChecked) }));
    ...
    render() {
        ...
        const items = this.state.martialStatus.filter((data) => {
            ...
            <div className="animated fadeIn">
                <Row className="justify-content-between align-items-center md-company-header mb-3">
                    <Col col="9" sm="9" md="9" xl >
                        <h5 className="content-sub-title mb-0">{t('daftar')} {t('statuspernikahan')}</h5>
                    </Col>
                    <Col col="3" sm="3" md="3">
                        <InputGroup>
...
export default connect(mapStateToProps)(translate(PengaturanDasarPernikahan));
